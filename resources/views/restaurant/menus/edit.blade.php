...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">Edit: {{ $menu->name }}</h2>
        <form method="POST" action="{{ route('restaurant.menus.update',$menu) }}" class="space-y-5">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">{{ old('description', $menu->description) }}</textarea>
            </div>
            <div>
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $menu->is_active) ? 'checked' : '' }} class="w-4 h-4 text-orange-500 rounded">
