...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">Edit Category</h2>
        <form method="POST" action="{{ route('restaurant.categories.update',$category) }}" class="space-y-5">
            @csrf @method('PUT')
            <div>
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="w-4 h-4 text-orange-500 rounded">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2.5 rounded-xl transition text-sm">Edit Category</button>
            </div>
        </form>
    </div>
</div>
