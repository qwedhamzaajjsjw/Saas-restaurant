...
@section('title', 'Previewing: '.$restaurant->name)

@section('content')

{{-- Preview Banner --}}
<div class="bg-purple-600 text-white rounded-2xl px-5 py-3 mb-6 flex items-center justify-between gap-4">
    <div class="flex items-center gap-3">
        ...
        <div>
            <p class="font-bold text-sm">Super Admin Preview Mode</p>
            <p class="text-xs text-purple-200">Viewing restaurant dashboard for: {{ $restaurant->name }}</p>
        </div>
    </div>
    <div class="flex items-center gap-3 flex-shrink-0">
        <a href="{{ $restaurant->storefront_url }}" target="_blank"
           class="text-xs bg-white/20 hover:bg-white/30 text-white border border-white/30 px-3 py-1.5 rounded-lg font-medium transition">
            View Storefront →
        </a>
        <a href="{{ route('admin.impersonate.stop') }}"
           class="text-xs bg-white text-purple-700 font-bold px-3 py-1.5 rounded-lg hover:bg-purple-50 transition">
            ← Back to Admin
        </a>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-gray-100 p-5 mb-6 flex items-center gap-4">
    ...
    @else
        <div class="w-14 h-14 bg-orange-100 rounded-xl flex items-center justify-center font-bold text-orange-600 text-2xl">
            {{ strtoupper(substr($restaurant->name, 0, 1)) }}
        </div>
    @endif
    <div>
        <h2 class="text-lg font-bold text-gray-800">{{ $restaurant->name }}</h2>
        <p class="text-sm text-gray-400">{{ $restaurant->email ?? $restaurant->phone ?? '—' }}</p>
    </div>
